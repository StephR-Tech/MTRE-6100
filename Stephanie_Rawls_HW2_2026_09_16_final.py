...
def confirmation(bel_x_t, sensor_reading, t):
...
def main():
    ...
    for i in range(2):
        ...
        confirmation(bel_x_t, sensor_reading,t)
         # Plotting the values for t = y 
        plot.figure(t)
        plot.xticks([0, 1, 2, 3])
        plot.yticks(np.arange(0,1,.1))
        plot.bar([0,1,2,3] ,bel_x_t )
        plot.ylabel("bel x(t)")
        plot.xlabel('Grid Positions')
        plot.title(f"Bayes Filter for Calculating Belief State at t ={t}" )
